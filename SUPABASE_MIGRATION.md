...
## 10. Mức án từng bị can — port từ `qlva.html`, chặn bởi thiếu cột Postgres (2026-07-21)

Theo yêu cầu Dũng: port tính năng "Mức án từng bị can" (đã làm trước đó trên nhánh `main`/
`qlva.html`, Firestore — xem CLAUDE.md) sang `qlahs-sup.html`/nhánh `supabase-migration` (nhánh
hiện là bản đang chạy thật theo Dũng xác nhận), CỘNG THÊM yêu cầu mới: Giao nhận hồ sơ (lúc nộp hồ
sơ lưu trữ, tính "Thời hạn bảo quản") giờ có **2 phương thức** — (a) NẾU vụ đã nhập mức án cho ít
nhất 1 bị can (qua "Hoàn thành vụ án"/"Sửa thông tin vụ án") thì TỰ ĐỘNG lấy bị can có mức án cao
nhất để tính, không cho nhập tay nữa; (b) nếu CHƯA bị can nào có mức án thì vẫn nhập tay cấp vụ như
trước (phương thức cũ, không đổi).

**Code JS đã port + mở rộng xong** (`qlahs-sup.html`, nhánh `muc-an-tung-bican-supabase` — tạo từ
`origin/supabase-migration`, CHƯA merge): 3 hàm/component dùng chung `moiMucAnTungBiCan`/
